...
class Game:
    """Internal engine for the stochastic multi-elevator MDP (RL setting).
    ...
    Compared with Assignment 2, the engine is unchanged in its dynamics,
    but the public API (GameAPI) no longer exposes the transition
    probabilities or the per-person reward lists -- they are hidden. The
    only new piece of information is `_last_gained_reward`, the reward
    obtained from the most recent action, which the student may use as a
    learning signal.
    """
    ...
    def __init__(self, problem, debug=False):
        ...
        # ---- HIDDEN model: used internally only, never exposed via GameAPI ----
    ...
    def submit_next_action(self, chosen_action):
        ...
        # reward gained from this action (0 if it yielded no reward).
        # On a delivery that completes the goal, this includes goal_reward.
        self._last_gained_reward = self._reward - reward_before
...
class GameAPI:
    """Public-facing wrapper around Game (RL setting).

    This is the ONLY class your controller is allowed to interact with.
    See the assignment PDF, Section "Engine API", for the engine-access
    policy. Any attempt to reach into the underlying Game object (e.g. via
    _GameAPI__game, vars(), __dict__, ...) is forbidden and will result
    in a grade of zero.

    NOTE (Assignment 3): the model is hidden. Unlike Assignment 2, this
    wrapper does NOT expose the elevator/person success probabilities or
    the per-person reward lists. The only reward signal you receive is
    get_last_gained_reward(), the reward obtained from the last action.
    """
    ...
    def get_last_gained_reward(self):
        return self.__game._last_gained_reward
